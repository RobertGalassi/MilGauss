...
% grafico 3D dell'andamento dei 3 lavori proiettati da un unico segmento0 da A a B
figure('Name','Visualizzazione 3D Lavoro Normalizzato');
hold on; view(3);
axis tight;

% Coordinate del segmento AB parametrizzato
t_AB = linspace(0, 1, 1000)';
x_AB = ax + t_AB*(bx - ax);
y_AB = ay + t_AB*(by - ay);

% Superfici trasparenti per i lavori
surface([x_AB, x_AB], [y_AB, y_AB], [zeros(size(Z_retto)), Z_retto],...
    'FaceColor', 'y', 'EdgeColor', 'none', 'FaceAlpha', 0.3);
surface([x_AB, x_AB], [y_AB, y_AB], [zeros(size(Z_semi)), Z_semi],...
    'FaceColor', [1 0.5 0], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
surface([x_AB, x_AB], [y_AB, y_AB], [zeros(size(Z_spirale)), Z_spirale],...
    'FaceColor', 'r', 'EdgeColor', 'none', 'FaceAlpha', 0.3);

% definisco le linee 3D per i lavori cumulativi
plot3(x_AB, y_AB, Z_retto, 'y', 'LineWidth', 2);
plot3(x_AB, y_AB, Z_semi, 'Color', [1 0.5 0], 'LineWidth', 2);
plot3(x_AB, y_AB, Z_spirale, 'r', 'LineWidth', 2);
plot3([ax bx], [ay by], [0 0], 'k--', 'LineWidth', 2); % Segmento AB

light('Style', 'infinite', 'Position', [1 1 1]);
lighting gouraud;
title(sprintf('Lavoro Cumulativo Normalizzato\nRetto: %.4f J | Semicerchio: %.4f J | Spirale: %.4f J',...
    W_retto, W_semi, W_spirale));
xlabel('X'); ylabel('Y'); zlabel('Lavoro Cumulativo (J)');
grid on;
rotate3d on;
